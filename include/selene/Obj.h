...
private:
    ...
    void _register_members(const detail::StateBlock &state, T *t) {}
...
public:
    Obj(const detail::StateBlock &state, T *t, Members... members) {
        lua_createtable(state.GetState(), 0, sizeof...(Members));
        _register_members(state, t, members...);
    }
};
}
