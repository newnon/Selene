...
private:
    ...
    void _register_ctor(const std::shared_ptr<lua_State> &state) {
        _ctor.reset(new A(state, _metatable_name.c_str()));
    }

    void _register_dtor(const std::shared_ptr<lua_State> &state) {
    ...
    void _register_members(const std::shared_ptr<lua_State> &state) {}
...
public:
    Class(const std::shared_ptr<lua_State> &state,
        ...
        luaL_newmetatable(state.get(), _metatable_name.c_str());
        _register_dtor(state);
        _register_ctor(state);
        _register_members(state, members...);
        lua_pushvalue(state.get(), -1);
        lua_setfield(state.get(), -1, "__index");
    }
    ~Class() {
